...
export class DictionaryCommand {
	...
	async dictionary(
		...
		if(status === 200) {
            ...
            embed
                .setTitle(`Dictionary | ${[..._words].join(", ")}`)
                .setDescription(`${[..._phonetics].join("   ")}\n\n\
                    Sources:\n${[..._sourceUrls].map(u=>`[${u}](${u})`).join("\n")}`);

            Object.entries(_meanings).forEach(([k, {definitions}]) => {
                let _definitions = definitions.map((_definition: {definition: string, example: string | undefined}) => {
                    var {definition, example} = _definition;
                    if(example) definition += `\n> *${example}*`;
                    return definition;
                }).join("\n\n");

                if(_definitions.length >= 1024)
                    _definitions = _definitions.slice(0, 1021) + "...";
