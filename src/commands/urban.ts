...
const URBAN_API_BASE =
	process.env.URBAN_API_BASE || "https://api.urbandictionary.com/v0/define?term=";
const URBAN_RANDOM = process.env.URBAN_RANDOM || "https://api.urbandictionary.com/v0/random";
const URBAN_DEFINE_BASE =
	process.env.URBAN_DEFINE_BASE || "https://urbandictionary.com/define.php?term=";
...
export class UrbanCommand {
	...
	makeDefinition(_definition: any): EmbedBuilder {
		...
		let _content = this.transformDefinition(definition),
			_example = example
				? "\n\n" +
				  this.transformDefinition(example)
						.split("\n")
						.map((l) => "> " + l)
						.join("\n")
				: "",
